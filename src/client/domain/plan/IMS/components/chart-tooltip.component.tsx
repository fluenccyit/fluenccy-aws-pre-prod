...
export const InvoiceChartTooltip = memo(({ props = {}, y, currency, datas, mode }: Props) => {
      ...
      <Card>
        <CardContent className="px-3 py-2 flex items-center whitespace-nowrap w-full justify-start" hasSeparator>
          ...
          <div className="text-sm mr-6 flex" style={{ minWidth: '120px' }} variant="gray">
            {mode === 'receivables' ? 'Wallet' : 'Currency Reserved'}
          </div>

          <Text className="text-sm font-medium" isBlock>
            {currencySymbol}
            {format(Number(item['Forward']), 2, 3)}
          </Text>
        </CardContent>
        <CardContent className="px-3 py-2 flex items-center whitespace-nowrap w-full justify-start" hasSeparator>
          <span style={{ width: '12px', height: '12px', backgroundColor: 'rgba(16, 188, 106, 0.25)', borderRadius: '50%' }} className="mr-4" />
          <div className="text-sm mr-6 flex" style={{ minWidth: '120px' }} variant="gray">
            Plan In Place
          </div>
          <Text className="text-sm font-medium" isBlock>
            {currencySymbol}
            {format(Number(item['Order/Spot']), 2, 3)}
          </Text>
        </CardContent>
        <CardContent className="px-3 py-2 flex items-center whitespace-nowrap w-full justify-start" hasSeparator>
          ...
          <Text className="text-sm font-medium" isBlock>
            {currencySymbol}
            {format(Number(item['Unmanaged']), 2, 3)}
          </Text>
        </CardContent>
        <CardContent className="px-3 py-2 flex items-center whitespace-nowrap w-full justify-start" hasSeparator>
          ...
          <Text className="text-sm font-bold" isBlock>
            {currencySymbol}
            {format(Number(item['Unmanaged']) + Number(item['Forward']) + Number(item['Order/Spot']), 2, 3)}
          </Text>
        </CardContent>
      </Card>
    </PopperContainer>
