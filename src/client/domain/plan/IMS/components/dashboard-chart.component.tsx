...
import { InvoiceChartTooltip } from '@client/plan/IMS/components';
...
export const DashboardChart = ({ chartBarClickable, isFullWidth, data = [], handleBarClick, currency, selectedDate, homeCurrency, mode }: Props) => {
  ...
  const formattedData = useMemo(() => {
    ...
    data.forEach((r, i) => {
      ...
      const b2 = isNaN(Number(r['Order/Spot'])) ? 0 : Number(r['Order/Spot']);
      ...
      bar2Data.push({ x: i + 1, y: b2, isLast: !b3, isFirst: !b1, date: r.dateDue, text: 'Plan In Place' });
      bar3Data.push({ x: i + 1, y: b3, isLast: !!b3, isFirst: !b1 && !b2, date: r.dateDue, text: 'No Plan' });
  ...
  }, [data, currency, mode, homeCurrency]);

  const BAR_WIDTH = isFullWidth ? 4 : 6;
  ...
  return (
    <div style={{ width: '100%' }}>
      <VictoryChart domainPadding={{ x: 20 }} padding={{ bottom: 20, left: 40, right: 0, top: 10 }} height={isFullWidth ? 140 : 180}>
        ...
        />
        <VictoryStack
          colorScale={[TAILWIND_THEME.colors.green['dark'], TAILWIND_THEME.colors.green['light'], TAILWIND_THEME.colors.gray['light']]}
          events={[
            {
              childName: 'all',
              target: 'data',
      ...
      {isHover && <InvoiceChartTooltip {...hoverDetails} datas={data} mode={mode} />}
    </div>
  );
};
