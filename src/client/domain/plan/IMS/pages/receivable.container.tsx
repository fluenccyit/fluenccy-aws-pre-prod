...
export const PlanReceivablePageContainer = memo(
  ({
    currency,
    duration,
    entitlements,
    ...
    }, [payable, managedInvoices, loading]);
    ...
    useEffect(() => {
      getInvoice();
      getInvoice('managed');
    }, [currency, duration, organisation?.id, homeCurrency, mode]);
    ...
    const getOptimisedPayableRate = (invoice, rates = null) => {
      ...
      if (diff > 0) {
        ...
        console.log('Actual Payable: ', payable);
        console.log('Current Rate: ', invoice.currentRate);

        // var newPayableRate = format(Number(-(format(Number(payable), 5,3) - format(Number(invoice.currentRate), 5,3)) + Number(invoice.currentRate)), 5, 3);
        ...
        console.log('New Payable Rate: ', newPayableRate);
    ...
    const getImpactInvoices = (data = []) => {
      const records = data.map((r) => {
        ...
        const clientSpotRate = format(Number(spotRate * (1 + calcMarginPercentage) + Number(forwardPoint?.forwardPoints || 0)), 5, 3);
        ...
        const lossOrGain = Number(r.total || 0) / clientSpotRate - Number(r.total || 0) / r.currencyRate;
    ...
    const renderOptimisedColumn = (r, column) => {
      if (column === 'actions') {
        ...
        return (
          <ActionTooltipContent data={data} onSelect={handleOptimisedInvoiceSelect.bind(this, r)} currency={r?.homeCurrencyCode} width="415px" />
        );
      }
        ...
        return (
          <span className="flex items-center " style={{ color: '#706A7A', fontWeight: 'bold' }}>
            {isNaN(r[column]) ? (
              ...
              <>
                <FlagIcon currency={r.homeCurrencyCode} style={cuurencyFlagStyle} /> {format(r[column], 2, 3)}
              </>
            )}
          </span>
        );
    ...
    const renderUnmanagedColumn = (r, column) => {
      switch (column) {
        ...
        case 'actions':
          ...
          return <ActionTooltipContent options={options} onSelect={handleOptimisedInvoiceSelect.bind(this, r)} currency={r?.homeCurrencyCode} />;
        case 'status':
    ...
    const unmanagedInvoiceColumns = [
      ...
      { key: 'dateDue', label: 'Expected Date', onRender: renderUnmanagedColumn, sortable: true, tooltip: 'Click to sort' },
    ...
    const memoManagedInvoices = useMemo(() => {
      return managedInvoices?.filter((i) => !i.isPricing) || [];
    }, [managedInvoices]);
    ...
    return showDashboard ? (
      <Dashboard
        ...
        onApprove={onApprove}
        organisation={organisation}
        managedInvoices={memoManagedInvoices}
