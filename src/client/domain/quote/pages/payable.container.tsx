...
import { isEmpty, differenceBy, orderBy, max, groupBy, isUndefined, uniqBy } from 'lodash';
import { localStorageService, useToast, useModal, FlagIcon, Badge, Input, APOLLO_ERROR_MESSAGE, Select } from '@client/common';
...
import { DAYS_FOR_CALCULATIONS, EDITABLE_COLUMNS, getStdDev, newInvoice } from '../utils';
...
export const QuotePayablePageContainer = memo(
  ({
    currency,
    duration,
    entitlements,
    ...
    filterBy,
    onDeleteNewRecord,
    onAddNew, // This should be passed from parent component
    showImpactAndSavings = false, // Add this prop
  }) => {
    ...
    useEffect(() => {
      getInvoice();
    }, [currency, organisation?.id, mode, showManaged, duration, filterBy]);

    useEffect(() => {
      if (generateQuote) {
        const invalidEntry = payable.some((r) => {
          let isAllFieldBlank = EDITABLE_COLUMNS.every((field) => !r[field]?.trim());
          if (isAllFieldBlank) return false;
          return EDITABLE_COLUMNS.some((field) => !r[field]);
        });
    ...
    useEffect(() => {
      if (!isEmpty(stdDev) && !isEmpty(currencies)) {
        const optimisedRates = payable?.reduce(
          (acc, invoice) => ({ ...acc, [`${invoice.invoiceId || invoice.id}`]: getOptimisedPayableRate(invoice) }),
          {}
        );
    ...
    const getImpactInvoices = (data = []) => {
      const records = data.map((r) => {
        ...
        console.log('payable clientSpotRate', clientSpotRate);
    ...
    }, [liveData, payable, optimisedRates, forwardPoints]);
    ...
    const renderSelectColumn = (r, column) => {
      if (r.type !== 'managed' && (r.isNew || editMode)) {
        const currencies = SUPPORTED_CURRENCIES.filter((c) => c != organisation.currency);
        return (
          <Select
            className="mt-2 px-1"
    ...
    // Add handlers for IMS and Delete actions (add these functions)
    ...
    // Update the renderColumn function to include IMS/CMS actions
    const renderColumn = (r, column) => {
      ...
      if (column === 'imsActions' && !showImpactAndSavings) {
        ...
        const isBeingEdited = editMode && EDITABLE_COLUMNS.some(field => 
      ...
      if (r.type !== 'managed' && (r.isNew || editMode) && EDITABLE_COLUMNS.includes(column)) {
        let defaultValue = r[column];
        if (column === 'total' && r[column]) {
          defaultValue = Number(r[column]).toFixed(2);
        }
        return (
          <input
    ...
    // Invoice NoID	Invoice Currency	Amount	Invoice Date	Due Date	Supplier	Budget rate	Days to pay	Current Cost	Forward Rate	Inverse FWD Rate	Gain / Loss	Target Cost	Savings Rate	Invers Savings Rate	Saving	Loss Reduction	Potential Loss	CTA 		Add New Line/Remove Line
    ...
    let editableColumns = [
      ...
      { key: 'total', label: 'Invoice Amount', onRender: renderColumn, isNumber: true },
      { key: 'dateDue', label: 'Due Date', onRender: renderDateColumn },
      ...
      { key: 'daysToPay', label: 'Days to Pay', isNumber: true, onRender: renderColumn },
    ...
    const renderEmptyState = () => {
      return (
        <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No Invoice Records Available</h3>

          <p className="text-gray-500 mb-8 max-w-md">
            Get started by adding your first invoice to begin managing your payment scheduling and currency optimization.
          </p>
